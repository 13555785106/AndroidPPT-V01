...
public class Camera2PhotoFragment extends Fragment
    ...
    int[] mFaceDetectModes;
    int mMaxFaceCount;
    Paint mMarkPaint = new Paint();
    Size mSensorPixelSize;
    ...
    private final TextureView.SurfaceTextureListener mFaceSurfaceTextureListener
            = new TextureView.SurfaceTextureListener() {

        @Override
        public void onSurfaceTextureAvailable(SurfaceTexture texture, int width, int height) {
            Canvas canvas = mFaceTextureView.lockCanvas();
        ...
        private void process(CaptureResult result) {
            ...
            switch (mState) {
                case STATE_PREVIEW: {
                    ...
                    Log.e(TAG, "width=" + mFaceTextureView.getWidth() + ",height=" + mFaceTextureView.getHeight());
                    Canvas canvas = mFaceTextureView.lockCanvas();
                    canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
                    if (faces != null) {
                        for (Face face : faces) {
                            ...
                            float scale = 0;
                            if (mFaceTextureView.getHeight() > mFaceTextureView.getWidth()) {
                                scale = mFaceTextureView.getHeight() * 1f / mSensorPixelSize.getWidth();
                            } else {
                                scale = mFaceTextureView.getWidth() * 1f / mSensorPixelSize.getWidth();
                            }
                            ...
                            if (mFaceTextureView.getHeight() > mFaceTextureView.getWidth()) {
                                canvas.drawLine(0, 0, rect.left, rect.top, mMarkPaint);
                                canvas.drawRect(rect.left, mSensorPixelSize.getHeight() - rect.top, rect.right, mSensorPixelSize.getHeight() - rect.bottom, mMarkPaint);
                            } else {
                                float aspectRatio = mFaceTextureView.getAspectRatio();
                                canvas.drawLine(0, 0, rect.left * aspectRatio, rect.top, mMarkPaint);
                                canvas.drawRect(rect.left * aspectRatio, mSensorPixelSize.getWidth() - rect.top - rect.width(), rect.right * aspectRatio, mSensorPixelSize.getWidth() - rect.bottom - rect.width(), mMarkPaint);
                            }
                        }
                    }
                    mFaceTextureView.unlockCanvasAndPost(canvas);
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        mFaceTextureView = view.findViewById(R.id.face_texture);
        mFaceTextureView.setAlpha(0.9f);
    ...
    public void onResume() {
        ...
        if (!mFaceTextureView.isAvailable()) {
            mFaceTextureView.setSurfaceTextureListener(mFaceSurfaceTextureListener);
        }
    }
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                mSensorPixelSize = characteristics.get(CameraCharacteristics.SENSOR_INFO_PIXEL_ARRAY_SIZE);//获取成像尺寸，同上
                Log.e(TAG, "mSensorPixelSize=" + mSensorPixelSize);

                mFaceDetectModes = characteristics.get(CameraCharacteristics.STATISTICS_INFO_AVAILABLE_FACE_DETECT_MODES);//支持的人脸检测模式
                if (mFaceDetectModes != null) {
                    String str = "";
                    for (int i = 0; i < mFaceDetectModes.length; i++) {
                        str += mFaceDetectModes[i];
                        if (i < mFaceDetectModes.length - 1)
                            str += ",";
                    }
                    Log.e(TAG, "最大人脸数量：" + str);
                }
                mMaxFaceCount = characteristics.get(CameraCharacteristics.STATISTICS_INFO_MAX_FACE_COUNT);//支持的最大检测人脸数量
                Log.e(TAG, "最大人脸数量：" + mMaxFaceCount);
                ...
                if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
                    ...
                    mFaceTextureView.setAspectRatio(mPreviewSize.getWidth(), mPreviewSize.getHeight());
                } else {
                    // 方向不是横向(即竖向)
                    mTextureView.setAspectRatio(mPreviewSize.getHeight(), mPreviewSize.getWidth());
                    mFaceTextureView.setAspectRatio(mPreviewSize.getHeight(), mPreviewSize.getWidth());
                }
                // 检查是否支持闪光.
                    ...
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
                            ...
                            try {
                                ...
                                mPreviewRequestBuilder.set(CaptureRequest.STATISTICS_FACE_DETECT_MODE, mFaceDetectModes[mFaceDetectModes.length - 1]);//设置人脸检测级别
                                // 自动闪光
    ...
    private void configureTransform(int viewWidth, int viewHeight) {
        ...
        mFaceTextureView.setTransform(matrix);
